...
_CHECKPOINT_FOR_DOC = "openai/whisper-large-v3"
...
class FlaxWhisperEncoderLayer(nn.Module):
    ...
    attention_implementation: str = "splash"

    def setup(self) -> None:
        self.embed_dim = self.config.d_model
        if self.attention_implementation == "splash":
            AttentionModule = FlaxWhisperAttentionSplash
        else:
            AttentionModule = FlaxWhisperAttention
...
class FlaxWhisperEncoderLayerCollection(nn.Module):
    ...
    def setup(self):
        self.layers = [
            FlaxWhisperEncoderLayer(self.config, name=str(i), dtype=self.dtype, params_dtype=self.params_dtype, attention_implementation=self.attention_implementation)
...
class FlaxWhisperDecoderLayerCollection(nn.Module):
    ...
    def setup(self):
        self.layers = [
            FlaxWhisperDecoderLayer(self.config, name=str(i), dtype=self.dtype, params_dtype=self.params_dtype, attention_implementation=self.attention_implementation)
...
class FlaxWhisperDecoder(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.layers = FlaxWhisperDecoderLayerCollection(self.config, dtype=self.dtype, params_dtype=self.params_dtype, attention_implementation=self.attention_implementation)
...
class FlaxWhisperModule(nn.Module):
    ...
    def setup(self) -> None:
        self.encoder = FlaxWhisperEncoder(self.config, dtype=self.dtype, params_dtype=self.params_dtype, attention_implementation=self.encoder_attention_implementation)
        self.decoder = FlaxWhisperDecoder(self.config, dtype=self.dtype, params_dtype=self.params_dtype, attention_implementation=self.decoder_attention_implementation)
...
class FlaxWhisperForConditionalGenerationModule(nn.Module):
    ...
    def setup(self) -> None:
        self.model = FlaxWhisperModule(
            ...
            dtype=self.dtype,
            params_dtype=self.params_dtype,
            encoder_attention_implementation=self.encoder_attention_implementation,
            decoder_attention_implementation=self.decoder_attention_implementation,
        )
...
class FlaxWhisperForConditionalGeneration(FlaxWhisperPreTrainedModel):
    ...
    def getInstance(cls):
        if cls._instance is None:
            raise RuntimeError("FlaxWhisperForConditionalGeneration instance has not been created yet.")
